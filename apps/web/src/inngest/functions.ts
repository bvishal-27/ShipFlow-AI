...
export const generatePRD = inngest.createFunction(
  { id: "generate-prd", name: "Generate PRD", triggers: [{ event: "shipflow/prd.generate" }] },
  async ({ event, step }: any) => {
    ...
    await step.run("update-status", async () => {
      return prisma.featureRequest.update({ where: { id: featureId }, data: { status: "PRD_GENERATING" } })
    })
    ...
    await step.run("save-prd", async () => {
      ...
      await prisma.pRD.create({ data: { featureRequestId: featureId, ...prdData, rawContent } })
      return prisma.featureRequest.update({ where: { id: featureId }, data: { status: "PRD_READY" } })
    })
    return { success: true }
  }
)
...
export const generateTasks = inngest.createFunction(
  { id: "generate-tasks", name: "Generate Tasks", triggers: [{ event: "shipflow/tasks.generate" }] },
  async ({ event, step }: any) => {
    const { featureId } = event.data
    const feature = await step.run("fetch-feature", async () => {
      return prisma.featureRequest.findUnique({ where: { id: featureId }, include: { prd: true } })
    })
    if (!feature?.prd) throw new Error("PRD not found")
    await step.run("update-status", async () => {
      return prisma.featureRequest.update({ where: { id: featureId }, data: { status: "PLANNING" } })
    })
    ...
    await step.run("save-tasks", async () => {
      ...
      return prisma.featureRequest.update({ where: { id: featureId }, data: { status: "READY_FOR_DEV" } })
    })
    return { success: true }
  }
)
...
export const runAIReview = inngest.createFunction(
  { id: "run-ai-review", name: "Run AI Review", triggers: [{ event: "shipflow/review.run" }] },
  async ({ event, step }: any) => {
    const { pullRequestId } = event.data
    const pr = await step.run("fetch-pr", async () => {
      return prisma.pullRequest.findUnique({
        where: { id: pullRequestId },
        include: { featureRequest: { include: { prd: true, tasks: true } } }
      })
    })
    if (!pr) throw new Error("PR not found")
    const connection = await step.run("fetch-connection", async () => {
      return prisma.githubConnection.findFirst({ where: { projectId: (pr.featureRequest as any).projectId } })
    })
    ...
    await step.run("create-review", async () => {
      ...
      return prisma.aIReview.create({ data: { pullRequestId, reviewNumber: count + 1, status: "RUNNING" } })
    })
    const diff = await step.run("fetch-diff", async () => {
      ...
      return (files as any[]).map((f: any) => `File: ${f.filename}\n${f.patch || ""}`).join("\n\n").slice(0, 8000)
    })
    const review = await step.run("ai-review", async () => {
      ...
      const { object } = await generateObject({
        model: groq("meta-llama/llama-4-scout-17b-16e-instruct"),
        schema: z.object({
          blockingIssues: z.array(z.object({ criterion: z.string(), issue: z.string(), suggestion: z.string() })),
          nonBlockingIssues: z.array(z.object({ area: z.string(), issue: z.string(), suggestion: z.string() })),
          summary: z.string(),
          passed: z.boolean(),
        }),
    ...
    await step.run("save-review", async () => {
      const latestReview = await prisma.aIReview.findFirst({ where: { pullRequestId }, orderBy: { createdAt: "desc" } })
      await prisma.aIReview.update({
        where: { id: latestReview!.id },
        data: { status: review.passed ? "PASSED" : "FAILED", blockingIssues: review.blockingIssues as any, nonBlockingIssues: review.nonBlockingIssues as any, summary: review.summary }
      })
      ...
      if (review.blockingIssues.length > 0) {
        ...
        await prisma.aIReview.update({ where: { id: latestReview!.id }, data: { postedToGithub: true } })
      }
    })
